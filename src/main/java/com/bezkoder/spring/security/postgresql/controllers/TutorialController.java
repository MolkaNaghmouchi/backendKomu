...
import com.bezkoder.spring.security.postgresql.restDto.TutorialDto;
  ...
  /*   @PostMapping("/add/{idTheme}")
    ...
    public Tutorial addTutorial(@RequestBody TutorialDto tutorialDto) {
        try {
            ...
            if(tutorialDto.getThemeIds()!=null){
                for (Long themeId : tutorialDto.getThemeIds()) {
                    Theme theme = themeRepository.findById(themeId)
                            .orElseThrow(() -> new RuntimeException("Thème introuvable avec l'ID " + themeId));
                    tutorial.getThemes().add(theme);
                }}else{
